...
if [ -f "$TASK_STATE" ]; then
    ...
    [ -n "$WAVE" ] && WAVE_DISPLAY=" | waves $WAVE"
...
if [ "${LAST_TOOL_USE:-0}" -ge 15 ] && [ "${LAST_COMMITS:-0}" -ge 1 ] && [ "$TREND_DOWN" -eq 0 ] 2>/dev/null; then
    RATING="❤️ Awesome"
elif [ "$TREND_UP" -ge 2 ] && [ "$TREND_DOWN" -eq 0 ] \
     && [ "${LAST_TOOL_USE:-0}" -ge 3 ] 2>/dev/null; then
    RATING="❤️ Awesome"
elif [ "${LAST_TOOL_USE:-0}" -ge 5 ] || [ "${LAST_COMMITS:-0}" -ge 1 ] || [ "$TREND_UP" -ge 1 ] 2>/dev/null; then
    RATING="😊 Good"
else
    RATING="😞 Do better"
fi

if [[ "$RATING" == "❤️ Awesome" ]] && [ "$TREND_DOWN" -gt 0 ] 2>/dev/null; then
    RATING="😊 Good"
fi
