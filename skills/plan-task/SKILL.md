...
> **Quick reference** — When to use: start of a new ticket (`/plan-task` or `/plan-task APIM-<N>`).
> Happy path: fetch ticket → orient on codebase → propose waves → parallel agents plan →
> compatibility check → assemble plan → design validation → PR strategy → Dev approval → write task-state.md.
> Output: `.claude/task-state.md` ready for `/implement-task`.
...
Re-spawn both agents simultaneously. Each receives: task description + its own template + the OTHER agent's full output as additional context.

Each agent must:
- Adopt the other's perspective where convinced (update its own sections accordingly)
- Explicitly flag any specific point it still disagrees on — do not silently discard a disagreement
- Produce a revised output ready for Phase 6

If both agents agree after seeing each other's work: proceed to Phase 6 with reconciled outputs.
If disagreements remain: they surface in `### Unresolved Disagreements` — Dev decides before Phase 7.
...
Ticket: APIM-<N>
Cached: <today's date>
PR Strategy: <one-pr-per-wave | one-pr-for-all>
