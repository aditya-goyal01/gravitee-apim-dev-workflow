...
#   §2  session-loader.sh        — 10 tests  (prompt-based)
#   §3  session-reviewer.sh      — 17 tests
#   §4  session-terminator.sh    — 12 tests  (task-state.md)
#   §5  plugin structure         —  8 tests
#   §6  task-state.md format     —  8 tests
...
make_metric() {
    ...
    local review_rounds="${10:-0}"
    local ticket="${branch#apim-}"
    printf '{"sessionId":"test","branch":"%s","ticket":"%s","timestamp":"%s","toolUseCount":%s,"duration":%s,"commitsCreated":%s,"filesChanged":%s,"workflowType":"%s","wavesCompleted":%s,"totalWaves":%s,"currentWave":%s,"prsMerged":0,"reviewRounds":%s}\n' \
        "$branch" "$ticket" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
        "$tool_use" "$duration" "$commits" "$files" "$workflow" \
        "$waves_done" "$total_waves" "$current_wave" "$review_rounds"
}
...
# 2.10 awaiting-review status → different context injected
git -C "$SL" checkout apim-1234 -q 2>/dev/null || git -C "$SL" checkout -b apim-1234 -q
make_task_state "$SL/.claude/task-state.md" 2 4 "awaiting-review" 1234 "Add filter"
OUT=$(echo "{\"cwd\":\"$SL\"}" | bash "$SESSION_LOADER" 2>/dev/null)
assert_contains "$OUT" "awaiting-review" "2.10 awaiting-review → context mentions awaiting-review"
assert_contains "$OUT" "PR is open" "2.10 awaiting-review → context says PR is open"
assert_contains "$OUT" "Nothing to implement" "2.10 awaiting-review → context says nothing to implement"

# 2.11 awaiting-review → workflow-type=awaiting_review written
WTYPE=$(cat "$SL/.claude/session-markers/workflow-type" 2>/dev/null || echo "")
[ "$WTYPE" = "awaiting_review" ] && pass "2.11 awaiting-review → workflow-type=awaiting_review" \
                                  || fail "2.11 workflow-type wrong: '$WTYPE' (want awaiting_review)"
rm -f "$SL/.claude/task-state.md"
...
# 3.8 Scoring — Awesome via shipped code + improving trend
rm -f "$METRICS"
make_metric "apim-1234" 3 1 2 200 "new_task" >> "$METRICS"
make_metric "apim-1234" 4 1 3 250 "continue" >> "$METRICS"
make_metric "apim-1234" 8 2 6 400 "continue" >> "$METRICS"
OUT=$(echo "{\"cwd\":\"$SR\"}" | bash "$SESSION_REVIEWER" 2>&1)
...
# 3.9 Scoring — Good (shipped code but declining trend)
rm -f "$METRICS"
make_metric "apim-1234" 20 5 15 800 "continue" >> "$METRICS"
make_metric "apim-1234" 3 1 2 200 "new_task" >> "$METRICS"
OUT=$(echo "{\"cwd\":\"$SR\"}" | bash "$SESSION_REVIEWER" 2>&1)
...
# 3.17 awaiting-review status in task-state → 🔄 shown in wave display
rm -f "$METRICS"
make_metric "apim-1234" 10 1 5 300 "continue" 1 4 2 >> "$METRICS"
make_task_state "$SR/.claude/task-state.md" 2 4 "awaiting-review" 1234 "Add filter"
OUT=$(echo "{\"cwd\":\"$SR\"}" | bash "$SESSION_REVIEWER" 2>&1)
assert_contains "$OUT" "🔄" "3.17 awaiting-review in task-state → 🔄 in wave display"
rm -f "$SR/.claude/task-state.md"

# 3.18 review rounds tip fires when prsMerged≥1 and reviewRounds≥2
rm -f "$METRICS"
printf '{"sessionId":"test","branch":"apim-1234","ticket":"1234","timestamp":"2026-01-01T00:00:00Z","toolUseCount":10,"duration":600,"commitsCreated":2,"filesChanged":5,"workflowType":"continue","wavesCompleted":1,"totalWaves":3,"currentWave":2,"prsMerged":1,"reviewRounds":3}\n' >> "$METRICS"
OUT=$(echo "{\"cwd\":\"$SR\"}" | bash "$SESSION_REVIEWER" 2>&1)
assert_contains "$OUT" "review round" "3.18 reviewRounds≥2 + prsMerged≥1 → review-rounds tip shown"

# 3.19 review rounds tip suppressed when reviewRounds=1
rm -f "$METRICS"
printf '{"sessionId":"test","branch":"apim-1234","ticket":"1234","timestamp":"2026-01-01T00:00:00Z","toolUseCount":10,"duration":600,"commitsCreated":2,"filesChanged":5,"workflowType":"continue","wavesCompleted":1,"totalWaves":3,"currentWave":2,"prsMerged":1,"reviewRounds":1}\n' >> "$METRICS"
OUT=$(echo "{\"cwd\":\"$SR\"}" | bash "$SESSION_REVIEWER" 2>&1)
assert_not_contains "$OUT" "review round" "3.19 reviewRounds=1 → no tip (normal approval)"
