...
│
├── hooks/
│   ├── session-loader.sh     ← prompt-based SessionStart, ~50 lines
│   ├── session-reviewer.sh   ← metrics display to stderr
│   ├── session-terminator.sh ← metrics append, marker cleanup
│   └── branch-manager.sh     ← utility called by session-loader (not a registered hook)
│
└── lib/
...
|--------|-------|---------|---------|
| `session-reviewer.sh` | SessionStart | `startup` | Print last session metrics + trend arrows to stderr |
| `session-loader.sh` | SessionStart | `startup` | Read task-state.md, inject branch + wave context to stdout |
| `session-terminator.sh` | SessionEnd | `""` | Grep wave progress from task-state.md, append JSONL metrics, clean markers |
| `branch-manager.sh` | — (utility) | — | Enforce `apim-<N>` branch naming, auto-stash on switch, restore offer |
...
- `lib/term.sh` — `TL`, `TOPT`, `TASK`, `TOK`, `TERR` (all to stderr)
...
(case-sensitive, two spaces before marker symbols: ✓ → ○).

**Analytics:** `.claude/session-metrics.jsonl` — one JSON line per session (branch, duration, tool use count, commits, files changed, wave progress, workflow type). Read by session-reviewer for trend arrows and rating.
